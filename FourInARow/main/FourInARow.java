...
public class FourInARow {    
    public static void main(String[] args) {
        ...
        try (Scanner scanner = new Scanner(System.in)) {
            ...
            while (playagain==1 || (!game.hasWon(p1) && !game.hasWon(p2))) { // P1 plays
                ...
                System.out.print("\n"+P1COLOR+"P1"+RESET+" choisis une colonne -> colonne 1->7: ");
                ...
                while (!ilachoisiunebonnecolonneP1) {
                    try {
                        ...
                        System.out.print("\n"+P1COLOR+"P1"+RESET+" choisis une  autre colonne car erreur avec celle-ci -"+RED+(colonne_P1+1)+RESET+"-  : ");
                        ...
                        while (colonne_mod == colonne_P1 && !ilachoisiunebonnecolonneP1) {
                            if (colonne_mod==colonne_P1){
                                System.out.println(P1COLOR+"P1"+RESET+" Veuillez choisir une autre colonne que "+RED+(colonne_P1+1)+RESET+" s'il vous plait:");
                                colonne_mod = scanner.nextInt()-1;
                            } else if (!ilachoisiunebonnecolonneP1){
                                System.out.println(P1COLOR+"P1"+RESET+" Veuillez choisir une autre colonne que "+RED+(colonne_P1+1)+RESET+" s'il vous plait! Cette colonne n'est pas valide. Choix: ");
                                colonne_mod = scanner.nextInt()-1;
                            } else {
                ...
                System.out.print("\n"+P2COLOR+"P2"+RESET+" choisis une colonne -> colonne 1->7: "); // P2 plays
                ...
                while (!ilachoisiunebonnecolonneP2) {
                    try {
                        ...
                        System.out.print("\n"+P2COLOR+"P2"+RESET+" choisis une  autre colonne car erreur avec celle-ci -"+RED+(colonne_P2+1)+RESET+"-  : ");
                        int colonne_mod = scanner.nextInt()-1;
                        //System.out.println(colonne_mod);
                        while (colonne_mod == colonne_P2 && !ilachoisiunebonnecolonneP2) {
                            if (colonne_mod==colonne_P2){
                                System.out.println(P2COLOR+"P2"+RESET+" Veuillez choisir une autre colonne que "+RED+(colonne_P2+1)+RESET+" s'il vous plait:");
                                colonne_mod = scanner.nextInt()-1;
                            } else if (!ilachoisiunebonnecolonneP2){
                                System.out.println(P2COLOR+"P2"+RESET+" Veuillez choisir une autre colonne que "+RED+(colonne_P2+1)+RESET+" s'il vous plait! Cette colonne n'est pas valide. Choix: ");
                                colonne_mod = scanner.nextInt()-1;
                            } else {
                                System.out.println(RED+"Message d'erreur : "+RESET);
                                colonne_mod = scanner.nextInt()-1;
    ...
    public static final String P1COLOR = "\033[32m"; // Green color
    public static final String BLUE = "\033[34m";  // Blue color
    public static final String YELLOW = "\033[33m";  // Yellow color
    public static final String P2COLOR = "\033[35m";  // Magenta color
    ...
    private static final String[] PLAYERS = {"_"+getP1COLOR()+"X"+RESET, "_"+getP2COLOR()+"O"+RESET};

    public static String getP2COLOR() {
        return P2COLOR;
    }
    ...
    public void play(int j, String player) { // j is the column index
        ...
        if (player.equals(PLAYERS[0]) || player.equals(PLAYERS[1])) {// 3)
            // valid column index?
            // column j not full?
            for (int i = 0; i < ROWS; i++) {
                if (i == 0 && !board[i][j].equals(EMPTY)) {
            ...
            for (int i = 0; i < ROWS; i++) { // 
                ...
                if (!board[i][j].equals(EMPTY) && i!=0 && board[i-1][j].equals(EMPTY)) {
                    board[i-1][j] = player;
                } else if (i==ROWS-1 && board[i][j].equals(EMPTY)) {
                    board[i][j] = player;
                }
            }
